...
int main(int argc, char** argv) {
    ...
    double frame_time = 0.0; // std::chrono::duration<double>(t_frame_end - t_frame_start).count();
    ...
    for(double t = 0.0; t < max_time; t += dt) {
        ...
        << " (" << std::setprecision(4) << std::setfill('0') << frame_time << "s/" << std::setprecision(4) << std::setfill('0') << mean_frame_time << "s)" <<
